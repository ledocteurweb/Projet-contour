...
void ecrire_image(Image I)
{
	...
	for(UINT haut=1 ;haut<=hauteur; haut++){
		for(UINT larg=1 ;larg<=largeur; larg++){
			printf("%d",get_pixel_image(I,larg,haut));
		}
		printf("\n");
...
Image negatif_image(Image I)
{
	...
	largeur = largeur_image(I);
	hauteur = hauteur_image(I);
	I1=creer_image(largeur,hauteur);
	for(UINT haut=1 ;haut<=hauteur; haut++){
		for(UINT larg=1 ;larg<=largeur; larg++){
			set_pixel_image(I1,larg,haut,(get_pixel_image(I,larg,haut)+1)%2);
		}
	}
	return I1;
}